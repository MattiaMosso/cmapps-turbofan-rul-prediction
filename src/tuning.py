...
def random_search_MLP(n_iter, dataset_name, train_set, test_set, y_test, train_size, sensor_names, seed,              
    ...
    lags_list = [0,1] # I choose not to tune the info of lagged variables
    ...
    for i in range(n_iter):
        ...
        train_RMSE, train_R2, val_RMSE, val_R2, computation_time = MD.run_MLP(dataset_name, train_set, test_set, y_test, train_size, sensor_names, seed,
            layer_sizes = layer_sizes, epochs = epochs, batch_size = batch_size, l2_param = l2_param, add_lagged_vars = lags, activation_function = activation_function, # hyperparameters that will be tested with RandomSearch              
        ...
        res = {
            ...
            'train_RMSE': train_RMSE, # metrics
            'train_R2': train_R2,
            'val_RMSE': val_RMSE,
            'val_R2': val_R2,
            'computation_time': computation_time,
            'layer_sizes': layer_sizes, # hyperparameters tuned with randomsearch
            'epochs': epochs,
            'l2_param': l2_param if l2_param != None else "no",
            'batch_size': batch_size,
            'lags': lags,
            'activation': activation_function,
            'scaling': scaling_method if apply_scaling != 0 else "no", # hyperparameters tuned manually
...
def random_search_LSTM(n_iter, dataset_name, train_set, test_set, y_test, train_size, sensor_names, seed,
    ...
    for i in range(n_iter):
        ...
        res = {
            ...
            'val_R2': val_R2,
            'computation_time': computation_time,
            'LSTM_layer_sizes': layer_sizes, # hyperparameters tuned with randomsearch
            'epochs': epochs,
            'l2_param': l2_param if l2_param != None else "no",
            'batch_size': batch_size,
            'sequence_length': sequence_length,
            'activation': activation_function,
            'scaling': scaling_method if (apply_scaling != 0 or apply_condition_scaling != 0) else "no", # hyperparameters tuned manually
            'clipping_threshold': clipping_threshold if apply_clipping != 0 else "no"
        }
...
def random_search_GRU(n_iter, dataset_name, train_set, test_set, y_test, train_size, sensor_names, seed,
    ...
    for i in range(n_iter):
        ...
        res = {
            ...
            'val_R2': val_R2,
            'computation_time': computation_time,
            'GRU_layer_sizes': layer_sizes, # hyperparameters tuned with randomsearch
            'epochs': epochs,
            'l2_param': l2_param if l2_param != None else "no",
            'batch_size': batch_size,
            'sequence_length': sequence_length,
...
def random_search_CNN1D(n_iter, dataset_name, train_set, test_set, y_test, train_size, sensor_names, seed,
    ...
    for i in range(n_iter):
        ...
        res = {
            ...
            'val_R2': val_R2,
            'computation_time': computation_time,
            'CNN_layer_sizes': CNN_layer_sizes, # hyperparameters tuned with randomsearch
...
def random_search_RF(n_iter, dataset_name, train_set, test_set, y_test, train_size, sensor_names, seed,              
    ...
    for i in range(n_iter):
        ...
        res = {
            ...
            'val_R2': val_R2,
            'computation_time': computation_time,
            'n_estimators': n_estimators, # hyperparameters
